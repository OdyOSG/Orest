...
convertSiToUs <- function(nameFromListOfMeasurements, siValue) {
  r <- data.table(getConv())[Measurement == nameFromListOfMeasurements, ][, res := round(siValue / Conversion, 2)]
...
convertUsToSi <- function(nameFromListOfMeasurements, usValue) {
  r <- data.table(getConv())[Measurement == nameFromListOfMeasurements, ][, res := round(usValue * Conversion, 2)]
